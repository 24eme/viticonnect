...
curl -s -D $headerfile "http://localhost:8080/cas/login?service=http://localhost/example&auto="$service_name > $outputfile ;
...
curl  -s -D $headerfile "http://localhost:8080/cas/serviceValidate?ticket="$ticket"&service=http://localhost/example"
echo
